  ...
  <div class="index">
    ...
    <!--预订列表日历-->
    <div class="date-box">
        <div class="font-14 business_affairs_d text_left">{{spotlist.name}}</div>
      ...
      <div class="Choose_a_room_dibu">
        <div class="Choose_a_room_dibu_c font-16 float_left">
          ...
          <span class="color-h font-12">
            ￥
            <i class="font-20" v-if="listyp.price!=null">{{listyp.price*adult+listyp.price*child}}</i>
             <i class="font-20" v-if="listyp.price==null">0</i>
          </span>
        </div>
        <button
...
.ht-rili-head .ht-rili-th {
  font-size: 10px !important;
   ...
   <script>
import { getScenicNumUrl } from "@/utils/getData";
import $ from "jquery";
import { async } from "q";
...
export default {
  ...
  data() {
    return {
      //选中价格
      scenicSpotTicketId:this.$route.params.scenicSpotTicketId,//门票id
      tyslit:1,//1常规路线2当地参团
      pricetyps: 0,//价格
      listyp:{},
      adult: 0, //大人
  ...
  methods: {
    ...
    nextclick: function() {
      if (store.state.loginUid != 0) {
        ...
        this.pricetyps=this.listyp.price*this.adult+this.listyp.price*this.child
    ...
    //日期接口
    async timtslit() {
      let data = await getScenicNumUrl(this.scenicSpotTicketId, this.priceDate);
      if (data) {
        let lists = [];
    ...
    activeChange() {
      this.dataList.map(list => {
        ...
        if (list.check == true) {
          this.listyp = list;
          this.startDate=this.listyp.date;
        }
      });
    },
    //日期初始化
